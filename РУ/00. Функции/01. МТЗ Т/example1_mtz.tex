
% Пример выбора уставок МТЗ Т <6,3 МВА

\item \textbf{Выбор уставок максимальной токовой защиты}

\begin{enumerate}[label=\arabic{section}.\arabic{enumi}.\arabic*, labelsep=4pt, leftmargin=0em, itemindent=65pt, parsep=3pt]
%\begin{enumerate}[label={}, labelsep=4pt, leftmargin=0pt, itemindent=57pt]
    
\item Выбор уставок ступени МТЗ
\begin{enumerate}[label=\arabic{section}.\arabic{enumi}.\arabic{enumii}.\arabic*, labelsep=4pt, leftmargin=0em, itemindent=65pt, parsep=0pt]
\item
Первичный ток срабатывания ступени $I_{\textsl{с.з.МТЗ}}^I$ определяется по условию отстройки от максимального рабочего тока трансформатора по выражению (\ref{eq:mtz1}):
\begin{equation*}
    I_{\textsl{с.з.МТЗ}}^I = \frac {k_{\textsl{н}} \cdot k_{\textsl{перег}}}{k_{\textsl{в}}}I_{\textsl{раб.макс.тр}} = \frac{1,2 \cdot 1,4}{0,95} \cdot 1,05 \cdot 62,4 = 116\; (\textsl{А}) ,
\end{equation*}

\item
Выдержка времени выбирается на ступень селективности больше времени срабатывания самой медленно действующей защиты питающей сети по выражению (\ref{eq:mtz4}):
\begin{equation*}
t_{\textsl{с.з.МТЗ}}^I = t_{\textsl{с.з.МТЗ}}^{\textsl{фид}}+{\Delta t} = 1,6 + 0,3 = 1,9\; (\textsl{с}) ,
\end{equation*}

\item
Чувствительность ступени МТЗ определяется по току двухфазного КЗ в минимальном режиме работы сети на шинах НН по выражению (\ref{eq:mtz5}):
\begin{equation*}
k_{\textsl{ч}} = \frac{I_{\textsl{к.мин.}}^{(2)}}{I_{\textsl{с.з. МТЗ}}^{I}} = \frac{0,87 \cdot 361}{116} = 2,7 > 1,3,
\end{equation*}
Ступень МТЗ удовлетворяет требованиям по чувствительности.

Уставка задается в относительных единицах от номинального тока аналогового входа устройства, для этого полученное значение пересчитывается по следующему выражению:
\begin{equation*}
    I_{\textsl{ср}} = \frac{I_{\textsl{с.з.МТЗ}}^I}{I_{\textsl{ном.вх.уст}}} \cdot \frac{I_{\textsl{ном.вт.ТТ}}}{I_{\textsl{ном.пер.ТТ}}} = \frac{116}{5} \cdot \frac{5}{600} = 0,19\; (\textsl{о.е.}),
\end{equation*}
\begin{eqexpl}[15mm]
    \item{$I_{\textsl{ном.вх.уст}}$} номинальный ток аналоговых входов устройства, 1 или 5 А;
    \item{$I_{\textsl{ном.вт.ТТ}}$} номинальный вторичный ток ТТ, А;
    \item{$I_{\textsl{ном.пер.ТТ}}$} номинальный первичный ток ТТ, А.
\end{eqexpl}
\item
Параметр \textbf{$I_{\textsl{ср}}$} <<Ток срабатывания>> ступени МТЗ принимается \textbf{равным 0,19}.
\item
Параметр \textbf{$T_{\textsl{ср}}$} <<Выдержка времени срабатывания>> ступени МТЗ принимается \textbf{равным 1,9 c}.

\end{enumerate}

\end{enumerate}
