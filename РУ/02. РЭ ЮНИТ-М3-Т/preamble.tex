...
%\usepackage[T1, T2A]{fontenc} % вроде как не нужны для lualatex
%\usepackage[utf8]{inputenc} % вроде как не нужны для lualatex
...
\usepackage{xcolor} % ПРИ КОМПИЛЯЦИИ В ЛИНУКС УБРАТЬ ЭТУ СТРОКУ, да вообще ее убрать, там дальше есть еще одно упоминание xcolor
\usepackage{colortbl}
...
%%%   СПЕЦИФИЧЕСКИЕ НАСТРОЙКИ МЕТОДИКИ ВЫБОРА УСТАВОК %%%%%%%%%%%%%%%%%%
\numberwithin{equation}{section} % для нумерации формул 1.1 

% НАСТРОЙКИ СПИСКА ЛИТЕРАТУРЫ
%https://www.linux.org.ru/forum/development/1230689
\makeatletter
\def\@biblabel#1{#1. }
\makeatother
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
{%
    \section*{\centering\color{uniblue} СПИСОК ИСПОЛЬЗУЕМЫХ ИСТОЧНИКОВ} % Заголовок по центру
    \addcontentsline{toc}{section}{Список используемых источников} % Добавляем в оглавление
    \begin{enumerate}[label=\arabic*., labelsep=4pt, leftmargin=0pt, itemindent=57pt] % Форматирование списка
    \setlength{\itemsep}{1.0ex} % Отступ между элементами
}{%
    \end{enumerate}
}
