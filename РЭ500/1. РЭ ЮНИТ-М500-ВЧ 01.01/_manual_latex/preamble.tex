...
%\setlist[itemize]{label=---, leftmargin=0cm, itemindent=1cm, labelsep=10pt}
%\setlist[itemize]{label=$\circ$} % окружность
% Сброс всех предыдущих настроек itemize
% ---------- Настройка для itemize (все уровни) ----------
\setlist[itemize]{
    label=-,
    labelwidth=-0.5em, % положение маркера
    labelsep=0.5em, % расстояние между маркером и текстом
    align=left,
    leftmargin=0em,      % = labelwidth + labelsep
    itemindent=3em, % отступ только для первой строки
    nosep
}
% при необходимости добавьте уровень 4

% ---------- Настройка для enumerate (все уровни) ----------
\setlist[enumerate,1]{
    labelwidth=5em,
    labelsep=0.5em,
    align=left,
...
% Если нужно, задайте формат меток для каждого уровня:
\setlist[enumerate,1]{label=\arabic*.}
\setlist[enumerate,2]{label=\arabic*.\arabic*.}
\setlist[enumerate,3]{label=\arabic*.\arabic*.\arabic*.}
% \setlist[enumerate,4]{label=\arabic*.\arabic*.\arabic*.\arabic*.}
%%%%%%%%%%%%%%%%%%%%%%%%
...
\usepackage{fontawesome7} % Подключить пакет шрифтов fontawesome
...
\makeatletter
\renewcommand{\p@subfigure}{\thefigure,\ }   % добавляет "0.2, " перед буквой
\renewcommand{\thesubfigure}{\asbuk{subfigure}}  % буква без скобки
\makeatother